...
const listarCategoria = async (req, res) => {
  const { usuario } = req;
  try {
    const categoriaPorUsuario = await knex("categorias")
      .where({ usuario_id: usuario.id })
      .returning("*");

    return res.status(200).json(categoriaPorUsuario);
  } catch (error) {
...
const adicionarCategoria = async (req, res) => {
  ...
  if (typeof nome != "string") {
    return res
      .status(400)
      .json({ mensagem: "O nome da categoria não pode ser um número" });
  }

  if (typeof descricao != "string") {
    return res
      .status(400)
      .json({ mensagem: "O descrição da categoria não pode ser um número" });
  }
  ...
  try {
    ...
    if (!novaCategoria) {
      return res
        .status(422)
        .json({ mensagem: "Não foi possível cadastrar a categoria" });
    }
  ...
  try {
    ...
    if (categoriaVinculada.length) {
      ...
      if (descricao.length === 1) {
        return res.status(422).json({
          mensagem:
            "A categoria selecionada não pode ser excluída, pois possui vínculo com a seguinte transação",
          descricao,
        });
      }
      return res.status(422).json({
        mensagem:
          "A categoria selecionada não pode ser excluída, pois possui vínculo com as seguintes transações",
        descricao,
      });
    }
    const categoriaExcluida = await knex("categorias")
      .where({ id: idCategoria, usuario_id: usuario.id })
      .del();
    ...
    if (!categoriaExcluida) {
      return res
        .status(400)
        .json({
          mensagem: "você não pode excluir as categorias padrão do sistema",
        });
    }
